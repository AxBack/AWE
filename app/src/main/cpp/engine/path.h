...
namespace Engine {
    ...
    private:
        ...
        typedef std::shared_ptr<Traversable> traversable_ptr;
        ...
        void add(float length, UINT nrControlPoints, const T* pControlPoints)
        {
            ...
            switch (nrControlPoints)
            {
                case 0:
                    return;
                case 1:
                    p = traversable_ptr(new Single(length, pControlPoints[0]));
                case 2:
                    p = traversable_ptr(new Linear(length, pControlPoints[0], pControlPoints[1]));
                    break;
                default:
                    p = traversable_ptr(new Bezier(length, nrControlPoints, pControlPoints));
                    break;
            }
    ...
    template <typename T>
    class Single : public Traversable
    {
    private:
    ...
    template <typename T>
    class Linear : public Traversable
    {
    private:
    ...
    template <typename T>
    class Bezier : public Traversable
    {
    private:
