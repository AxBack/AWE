...
void ConnectionsUpdater::advance(float dt)
{
    if(m_points.size() == 0)
        return;
...
bool ConnectionsUpdater::updateInstances(Mesh<Vertex, PointInstance>& pointMesh,
...
void ConnectionsUpdater::add(Vector3 point, Vector3 color, float size, UINT connectedTo)
{
...
void ConnectionsUpdater::addConnection(UINT i1, UINT i2, float minDistance, float maxDistance)
{
...
void ConnectionsUpdater::rayTest(const Vector3& start, const Vector3& end)
{
    std::lock_guard<std::mutex> _(m_eventMutex);
